...
class Evaluation():
    def __init__(self, infer_model, in_dir, out_dir, score_path, para_num, eval_model="gpt4o", temperature=0.01):
        ...
        self.in_path = os.path.join(in_dir, f"{infer_model}_infer.jsonl")
    ...
    def _get_judge_scores(self, task):
        ...
        try:
            ...
            for idx, judge in enumerate(judges):
                ...
                assert crit.strip() == criteria[idx].strip()            # 判断3:gpt4评估的返回结果中criteria和原始task中的criteria对不上;
    ...
    def _get_judge_scores2(self, task):
        judge_response = task["judge_response"]
        criteria = [crit[0] for crit in task["criteria"]]
        scores = []
        try:
    ...
    def _judged(self, tasks, para_num):
        ...
        for _ in range(MAX_RETRY_NUM):
            ...
            for task in tasks_judged_:
                ...
                elif self._get_judge_scores2(task):
                    tasks_judged.append(task)
                    task['judge_score'] = self._get_judge_scores2(task)
                    task['judge_parsing'] = "1"
                else:
    ...
    def _scores(self, results):
        ...
        CSR, ISR, PSR = round(np.mean(CSRs),4), round(np.mean(ISRs),4), round(np.mean(PSRs),4)
        CSR_e, ISR_e, PSR_e = round(np.mean(CSRs_easy),4), round(np.mean(ISRs_easy),4), round(np.mean(PSRs_easy),4)
        CSR_h, ISR_h, PSR_h = round(np.mean(CSRs_hard),4), round(np.mean(ISRs_hard),4), round(np.mean(PSRs_hard),4)
    ...
    def __call__(self):
        ...
        data = self.read_jsonl(self.in_path)
