...
def run_inference(args):
    ...
    for constraint_type in args.constraint_types:
        ...
        os.makedirs(f"{args.api_output_path}/{args.model_path.split('/')[-2]}", exist_ok=True)
        with open(f"{args.api_output_path}/{args.model_path.split('/')[-2]}/{constraint_type}_constraint.jsonl", 'w', encoding='utf-8') as output_file:
